...
import com.kcsfsoft.cs667ble.ble.PERMISSION_BLUETOOTH_CONNECT
import com.kcsfsoft.cs667ble.ble.PERMISSION_BLUETOOTH_SCAN
    ...
    Column(
        ...
        if (isScanning) {
            Column(
                ...
                Button(onClick = stopScanning, modifier = Modifier.padding(top = 8.dp)) {
                    Text("Stop Scanning")
                }
            }
        } else {
            Column(
                ...
                Button(onClick = startScanning, modifier = Modifier.padding(top = 8.dp)) {
                    Text("Start Scanning")
                }
            }
        }
        ...
        LazyColumn(
            ...
            items(foundDevices) { device ->
                DeviceItem(deviceName = device.name, selectDevice = { selectDevice(device) })
            }
        }
    }
}
